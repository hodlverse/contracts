...
describe("Timelock", () => {
  ...
  let revertData = encodeParameters(
    ["uint256"],
    [etherUnsigned(60 * 60).toFixed()]
  );
  let eta;
  let queuedTxHash;
  ...
  describe("acceptAdmin", () => {
    ...
    it("sets pendingAdmin to address 0 and changes admin", async () => {
      await send(timelock, "harnessSetPendingAdmin", [newAdmin], {
        from: root,
      });
      ...
      expect(pendingAdminAfter).toEqual(
  ...
  describe("queueTransaction", () => {
    ...
    it("requires eta to exceed delay", async () => {
      ...
      await expect(
        send(
          timelock,
          "queueTransaction",
          [target, value, signature, data, etaLessThanDelay],
          {
            from: root,
          }
        )
    ...
    it("sets hash as true in queuedTransactions mapping", async () => {
      const queueTransactionsHashValueBefore = await call(
        timelock,
        "queuedTransactions",
        [queuedTxHash]
      );
      ...
      const queueTransactionsHashValueAfter = await call(
        timelock,
        "queuedTransactions",
        [queuedTxHash]
      );
  ...
  describe("queue and cancel empty", () => {
    it("can queue and cancel an empty signature and data", async () => {
      ...
      await send(timelock, "queueTransaction", [target, value, "", "0x", eta], {
        from: root,
      });
  ...
  describe("executeTransaction (setDelay)", () => {
    beforeEach(async () => {
      ...
      await send(
        timelock,
        "queueTransaction",
        [target, value, signature, revertData, eta],
        {
          from: root,
        }
      );
    ...
    it("requires transaction to be queued", async () => {
      ...
      await expect(
        send(
          timelock,
          "executeTransaction",
          [target, value, signature, data, differentEta],
          { from: root }
        )
      ).rejects.toRevert(
  ...
  describe("executeTransaction (setPendingAdmin)", () => {
    ...
    it("sets hash from true to false in queuedTransactions mapping, updates admin, and emits ExecuteTransaction event", async () => {
      ...
      expect(configuredPendingAdminBefore).toEqual(
        "0x0000000000000000000000000000000000000000"
      );
