...
describe("Factory", function () {
    ...
    it("should allow dev and only dev to update dev", async function () {
        await expect(this.factory.connect(this.bob).setOwner(this.carol.address)).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).toggleLpFee()).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).setMigrator(this.carol.address)).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).setBuyback(this.buyback.address)).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).setMoney(this.money.address)).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).setSwapFee(1)).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).setBuybackShare(1)).to.be.revertedWith("HODL: FORBIDDEN");
        await expect(this.factory.connect(this.bob).setDiscountEligibilityBalance(1)).to.be.revertedWith("HODL: FORBIDDEN");
