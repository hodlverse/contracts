...
describe("MultiSigWallet", () => {
  ...
  beforeEach(async () => {
    [accounts0, accounts1, accounts2, accounts3, accounts4] =
  ...
  it("test execution after requirements changed", async () => {
    ...
    let addOwnerData = iface.encodeFunctionData("addOwner", [
      accounts3.address,
    ]);
    ...
    await expect(
      multisigInstance
        .connect(accounts0)
        .submitTransaction(multisigInstance.address, 0, addOwnerData)
    )
    ...
    const updateRequirementData = iface.encodeFunctionData(
      "changeRequirement",
      [newRequired]
    );
    ...
    await expect(
      multisigInstance.connect(accounts0).revokeConfirmation(transactionId2)
    )
    ...
    await multisigInstance
      .connect(accounts1)
      .confirmTransaction(transactionId1);
    ...
    ).to.equal(transactionId0);

    // Execution fails, because sender is not wallet owner
    await expect(
      multisigInstance.connect(accounts4).executeTransaction(1)
    ).to.be.revertedWith("MultiSigWallet::ownerExists: ERR_ONWER_EXIST");

    // Because the # required confirmations changed to 1, the addOwner transaction can be executed now
    await expect(
      multisigInstance.connect(accounts0).executeTransaction(transactionId0)
    )
    ...
    expect(
      await multisigInstance
        .connect(accounts1)
        .getConfirmationCount(transactionId1)
    ).to.equal(2);
    expect(
    ...
    expect(await multisigInstance.getOwners()).to.have.all.members([
      accounts0.address,
      accounts1.address,
      accounts2.address,
      accounts3.address,
    ...
    expect(
      await multisigInstance.connect(accounts1).getConfirmations(transactionId1)
    ).to.have.all.members([accounts0.address, accounts1.address]);

    ABI = ["function replaceOwner(address owner, address newOwner)"];
    iface = new ethers.utils.Interface(ABI);
    const replaceOwnerData = iface.encodeFunctionData("replaceOwner", [
      accounts0.address,
      accounts4.address,
    ]);
    await expect(
      multisigInstance
        .connect(accounts0)
        .submitTransaction(multisigInstance.address, 0, replaceOwnerData)
    )
    ...
    iface = new ethers.utils.Interface(ABI);
    const removeOwnerData = iface.encodeFunctionData("removeOwner", [
      accounts2.address,
    ]);
    ...
    await expect(
      multisigInstance
        .connect(accounts2)
        .submitTransaction(multisigInstance.address, 0, removeOwnerData)
    )
      .to.emit(multisigInstance, "OwnerRemoval")
